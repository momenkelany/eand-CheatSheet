...
async function generate() {
  ...
  // Resize logo to a width of 700 (so it is large enough for 1200x630 canvas)
  const logoResized = await sharp(svgBuf).resize({ width: 700 }).png().toBuffer();
  const { width: logoW, height: logoH } = await sharp(logoResized).metadata();

  const canvasW = 1200;
  const canvasH = 630;

  const left = Math.floor((canvasW - (logoW ?? 700)) / 2);
  const top = Math.floor((canvasH - (logoH ?? 200)) / 2);
